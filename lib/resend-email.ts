...
const VERIFICATION_EMAIL_FROM =
  process.env.EMAIL_FROM || "LaunchRecord <hello@launchrecord.com>";
const APP_URL =
  process.env.NEXT_PUBLIC_APP_URL || "https://www.launchrecord.com";
...
export async function sendVerificationEmail(
  ...
  const { data, error } = await resend.emails.send({
    ...
    to: [email],
    subject: "Verify Your Email - Founderego",
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
export async function sendPasswordResetEmail(
  ...
  const { data, error } = await resend.emails.send({
    from: VERIFICATION_EMAIL_FROM,
    to: [email],
    subject: "Reset Your Password - Founderego",
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
export async function sendClaimVerificationLink(
  email: string,
  ...
  const { error } = await resend.emails.send({
    from: VERIFICATION_EMAIL_FROM,
    to: [email],
    subject: `Claim Your Record - ${productName}`,
    template: {
      id: "ownership-veirifaction", // The ID or alias of your published template
      variables: {
        OWNERSHIP_VERIFICATION_LINK: verificationUrl,
        PRODUCT_NAME: "Vintagexc",
      },
    },
  });

  if (error) {
    console.error("Failed to send claim code via Resend:", error);
    throw new Error("Failed to send claim verification code");
  }
