...
import { Lightbulb, Lock, ShieldAlert, ShieldX, Sparkles } from "lucide-react";
...
export function IssueCard({
      ...
      case "critical":
        return {
          ...
          badge: (
            <Badge
              variant="outline"
              className="bg-red-600 text-white border-none px-2 py-0 h-5 text-[10px] font-black uppercase"
            >
              <ShieldX className="w-3 h-3 mr-1" /> Critical Block
            </Badge>
          ),
        };
      case "high":
        return {
          ...
          badge: (
            <Badge className="bg-orange-500 text-white border-none px-2 py-0 h-5 text-[10px] font-black uppercase">
              <ShieldAlert className="w-3 h-3 mr-1" /> High Friction
            </Badge>
          ),
        };
      case "medium":
        return {
          ...
          badge: (
            <Badge
              variant="outline"
              className="bg-yellow-400 text-yellow-900 border-none px-2 py-0 h-5 text-[10px] font-black uppercase"
            >
              Medium Impact
            </Badge>
          ),
      ...
      default:
        return {
          ...
          badge: (
            <Badge
              variant="outline"
              className="bg-blue-500 text-white border-none px-2 py-0 h-5 text-[10px] font-black uppercase"
            >
              Optimization
            </Badge>
          ),
  ...
  return (
    <Card
      className={`group relative border shadow-sm transition-all duration-300 rounded-xl overflow-hidden ${styles.border} ${isLocked ? "opacity-60" : "hover:shadow-md"}`}
    >
      {/* Visual Accent */}
      ...
      <div className="p-5 space-y-4">
              ...
              <div className="space-y-2.5">
                ...
                <div className="space-y-3">
                  {issue.fixes.map((fix, idx) => (
                    <div
                      key={idx}
                      className="text-sm text-emerald-800 bg-emerald-50/30 border border-emerald-100/50 p-4 rounded-xl font-bold italic leading-relaxed"
                    >
                      "{fix}"
                    </div>
                  ))}
        ...
        {isLocked && (
          <div className="flex items-center gap-2 text-[10px] text-slate-500 font-medium pt-2 italic">
            <Lock className="w-3 h-3" /> Upgrade to unlock detailed
            recommendations
          </div>
        )}
      </div>
