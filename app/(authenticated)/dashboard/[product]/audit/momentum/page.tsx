...
import { ArrowLeft, TrendingUp, Rocket, Flame, BarChart3, Activity, Zap, Award } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500">Is your startup moving or stagnating?</p>
          </div>
        </div>
        <Button
        ...
        <div className="relative z-10">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-bold mb-2">Growth Trajectory Analysis</h2>
              <p className="text-orange-100">
                Measure your startup's velocity, market signals, and traction evidence. 
                Momentum attracts customers, talent, and capital.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-end gap-2 mb-6 h-24">
            {[20, 40, 60, 80, 100].map((level, idx) => (
              <div
                ...
                style={{
                  height: `${level}%`,
                  background: level <= 20 ? '#ef4444' : level <= 40 ? '#f97316' : level <= 60 ? '#eab308' : level <= 80 ? '#22c55e' : '#10b981',
                }}
              />
            ))}
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Where does your startup fall on the growth spectrum?</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-5 gap-3">
            {[
              { name: "Viral", score: "90-100", desc: "Exponential growth", gradient: "from-emerald-500 to-green-600" },
              { name: "Rising", score: "70-89", desc: "Strong upward", gradient: "from-blue-500 to-indigo-600" },
              { name: "Stable", score: "50-69", desc: "Steady growth", gradient: "from-yellow-500 to-amber-600" },
              { name: "Flat", score: "30-49", desc: "Minimal growth", gradient: "from-orange-500 to-red-600" },
              { name: "Dead", score: "0-29", desc: "No momentum", gradient: "from-red-600 to-rose-800" },
            ].map((band, idx) => (
              <div
                key={idx}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Six signals that reveal your trajectory</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-2 gap-4">
            {[
              { icon: <Rocket className="h-5 w-5" />, label: "Growth Trajectory", desc: "Direction and velocity", color: "text-blue-600 bg-blue-100" },
              { icon: <BarChart3 className="h-5 w-5" />, label: "Market Signals", desc: "Press, partnerships", color: "text-indigo-600 bg-indigo-100" },
              { icon: <Activity className="h-5 w-5" />, label: "User Engagement", desc: "Active usage metrics", color: "text-purple-600 bg-purple-100" },
              { icon: <Flame className="h-5 w-5" />, label: "Competitive Velocity", desc: "Growth vs competitors", color: "text-orange-600 bg-orange-100" },
              { icon: <Award className="h-5 w-5" />, label: "Traction Evidence", desc: "Metrics, logos, wins", color: "text-yellow-600 bg-yellow-100" },
              { icon: <Zap className="h-5 w-5" />, label: "Velocity Indicators", desc: "Acceleration signals", color: "text-red-600 bg-red-100" },
            ].map((item, idx) => (
              <div key={idx} className="flex items-start gap-3 p-4 rounded-xl border border-slate-200 hover:border-orange-300 transition-colors">
      ...
      <Card className="border-orange-200 bg-orange-50">
        <CardHeader>
          <CardTitle className="text-orange-900">Momentum Diagnostics</CardTitle>
        </CardHeader>
        <CardContent>
