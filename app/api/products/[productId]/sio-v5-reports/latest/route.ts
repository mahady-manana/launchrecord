...
export async function GET(
  ...
  try {
    ...
    const latestReport = await SIOReport.findOne({ product: productId })
      .sort({ createdAt: -1 })
      .lean();
    ...
    console.error("Error fetching latest SIO-V5 report:", error);
    return NextResponse.json(
      { error: "Failed to fetch latest SIO-V5 report" },
      { status: 500 },
    );
  }
}
