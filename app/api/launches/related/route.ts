...
export async function GET(request: Request) {
  try {
                    ...
                    $and: [
                      ...
                      { $isArray: [parsedCategory] },
                      {
                        $gt: [
                          {
                            $size: {
                              $setIntersection: ["$category", parsedCategory],
                            },
                          },
                          0,
                        ],
    ...
    let similarApps: LaunchType[] = [];
    let newApps: LaunchType[] = [];
    let topLaunches: LaunchType[] = [];

    if (results.length > 0) {
      const allLaunches: LaunchType[] = results[0].allLaunches;
      ...
      topLaunches = allLaunches
        .filter((launch) =>
          ["hero", "left", "right"].includes(launch.placement),
        )
        .slice(0, 6);

      // If not enough top launches, fill with others
      if (topLaunches.length < 6) {
        const remainingLaunches = allLaunches.slice(0, 6 - topLaunches.length);
